...
**Omnibar / revset-first command bar (M3 or earlier).** The fuzzy-find modal (`/`) evolves into a revset-first command bar. Input is first passed to jj's revset parser — if it's a valid expression (e.g., `mine() & ~abandoned()`), `LoadGraph` fires with the custom revset and the graph re-roots. If it's an invalid revset, it falls back to fuzzy-matching on descriptions and IDs. UI: a persistent `[ / ] Search or Revset...` hint in the status bar that expands into a full-width input. M2 implication: `Effect::LoadGraph` takes an optional revset parameter (`Option<String>`) from the start, defaulting to `None` (use hardcoded default). This avoids a signature change later.

**Context-aware conflict mode (M4).** When the selected change has `has_conflict`, the detail pane conditionally swaps its file list for a "conflict manager" layout: filtered to conflicted files only, per-file resolved/unresolved status. Enter triggers the 3-way merge UI or launches the user's configured merge tool (not just a diff view). This is a `DetailMode::ConflictView` that activates automatically based on `ChangeDetail.has_conflict`. M2 implication: none — the `DetailMode` enum is extensible, and the graph already carries `has_conflict` per change. Conflict data passes back through `GraphLoaded` as normal.

**The interrupt pattern / effect cancellation (M3+).** The current effect model has no cancellation. For M2's push/fetch this is acceptable. Later milestones (background revset evaluation, conflict detection) need it. The design:
- **Task identity:** Every background effect includes a `TaskCategory` (e.g., `Diff`, `Fetch`, `RevsetEval`).
- **Auto-cancellation:** The executor maintains a map of active tasks. If a new task arrives for a singleton category (like `Diff`), the executor drops the previous `TaskHandle` before starting the new one. This handles rapid navigation while a diff is loading — no stale results arrive.
