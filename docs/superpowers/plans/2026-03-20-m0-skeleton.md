...
**Tech Stack:** Rust 1.85+ (edition 2024), ratatui 0.30, crossterm 0.28, anyhow
...
mod tests {
    ...
    fn initial_cursor_fallback_without_working_copy() {
        ...
        // Falls back to first node line
        assert_eq!(state.cursor, 0);
    }

    #[test]
    fn refresh_falls_back_when_change_disappears() {
        // Start with a graph where cursor is on "def"
        let mock_graph = sample_graph();
        let mock = MockBackend { graph: mock_graph };
        let mut state = AppState::new(sample_graph());
        state.cursor = 2; // at "def"

        // Simulate a refresh where "def" no longer exists
        let mut new_graph = sample_graph();
        new_graph.lines.remove(3); // remove def's continuation line
        new_graph.lines.remove(2); // remove def's node line
        new_graph.details.remove("def");
        // Recompute: ghi is now at index 2
        let new_mock = MockBackend { graph: new_graph };

        dispatch(&mut state, Action::Refresh, &new_mock);
        // Should fall back to working copy (index 0)
        assert_eq!(state.cursor, 0);
        assert_eq!(state.selected_change_id(), Some("abc"));
    }
...
mod tests {
    ...
    fn jump_keys() {
        ...
        assert_eq!(map_event(key(KeyCode::Char('G'))), Some(Action::JumpToBottom));
    }
...
mod tests {
    ...
    fn highlighted_lines_have_reversed_style() {
        ...
        assert_eq!(buf[(0, 0)].style(), highlight);
        assert_eq!(buf[(0, 1)].style(), highlight);
        // Line 2 (different change) should not
        assert_ne!(buf[(0, 2)].style(), highlight);
    }
}
```
...
fn main() -> Result<()> {
    // Discover workspace (before entering alternate screen so errors are visible)
    ...
    // Set up terminal (ratatui::init handles raw mode, alternate screen, and panic hook)
    let mut terminal = ratatui::init();

    // Event loop
    let result = run_loop(&mut terminal, &mut state, &backend);

    // Restore terminal (ratatui::restore handles raw mode and alternate screen)
    ratatui::restore();
...
- [ ] **Step 2: Verify dependencies are correct**

`crates/lajjzy-cli/Cargo.toml` should already have `lajjzy-core`, `lajjzy-tui`, `ratatui`, `crossterm`, and `anyhow` from Task 1. Verify they are present.
